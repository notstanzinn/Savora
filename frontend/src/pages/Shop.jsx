...
function Shop() {
  ...
  return (
    <div className='min-h-screen bg-gray-50'>
        <button onClick={()=>navigate("/")} className='absolute top-4 left-4 z-20 flex items-center gap-2 bg-black/50 hover:bg-black/70 text-white px-3 py-2 rounded-full shadow transition'><FaArrowLeft /><span>Back</span></button>
      {shop && 
        <div className='relative h-64 w-full md:h-80 lg:h-96'>
            ...
            <div className='absolute inset-0 bg-linear-to-b from-black/70 to-black/30 flex flex-col justify-center items-center text-center px-4'>
                ...
                <h1 className='text-white drop-shadow-lg font-extrabold text-3xl md:text-5xl'>{shop.name}</h1>
                <div className='flex items-center gap-2.5'>
                    <FaLocationDot size={22} color='red'/>
                    <p className='mt-2.5 text-lg text-gray-200 font-medium'>{shop.address}</p>
                </div>
